...
The dual-index design consists of a DPR (Dense Passage Retrieval) index backed by FAISS and a ChromaDB dense vector index. These are separate data stores queried in parallel at retrieval time. DPR is used instead of BM25 sparse lexical retrieval because dense retrieval has been shown to significantly outperform BM25 on open-domain question answering benchmarks, achieving 65.2% top-20 passage retrieval accuracy compared to 59.1% for BM25 on Natural Questions (Karpukhin et al., 2020). Legal retrieval similarly benefits from dense representations because legal language is highly paraphrastic — the same legal principle can be expressed with entirely different vocabulary across judgments, which BM25's term-matching approach fails to capture. DPR uses separate question and context encoders (facebook/dpr-question_encoder-single-nq-base and facebook/dpr-ctx_encoder-single-nq-base) producing 768-dimensional embeddings, stored in a FAISS IndexFlatIP for exact inner-product search. The index is persisted to disk and loaded at query time. ChromaDB is configured with the all-MiniLM-L6-v2 embedding model from sentence-transformers. This model produces 384-dimensional embeddings and is CPU-compatible, making it suitable for the compute-flexible requirement. Embeddings are computed in batches of 32 during indexing and stored in ChromaDB with HNSW indexing for approximate nearest neighbor search.
...
Hybrid fusion combines the DPR and ChromaDB dense retrieval result lists using Reciprocal Rank Fusion. RRF is the correct choice because the two dense indexes use different embedding models and similarity metrics — DPR uses inner-product similarity over 768-dimensional embeddings while ChromaDB uses cosine similarity over 384-dimensional all-MiniLM-L6-v2 embeddings — making their raw scores incommensurable. Weighted score combination requires normalizing both score ranges and choosing a weight hyperparameter, which is data-dependent and unstable. RRF instead operates on rank positions, which are directly comparable. The RRF formula is: score(chunk) = sum over all retrieval lists of (1 / (k + rank(chunk))), where k is a small constant (typically 60) that reduces the impact of high-rank outliers. Each chunk's RRF score is computed by summing its rank-based contribution from the DPR list and the ChromaDB list. Chunks that appear in both lists receive contributions from both and are ranked higher. Chunks that appear in only one list still receive a contribution from that list and may rank in the top K if their single-list rank is high enough.
...
For each node in the tree, the full hybrid retrieval pipeline is executed. The retrieval query is constructed from the fact sheet at that node by concatenating the party types, the sections cited, a summary of the numerical facts, and the evidence items into a structured query string. This query string is passed through the pre-filter, DPR and ChromaDB ANN search, RRF fusion, MMR re-ranking, and cross-encoder re-ranking as described in Section 6. The top K result set is stored at the node along with the fact sheet state and the edge description from the parent.
...
[Dual Indexing]
    DPR Index:
        Tool: facebook/dpr-ctx_encoder-single-nq-base + FAISS IndexFlatIP
        Input: chunk text → 768-dim dense embeddings
        Output: FAISS inner-product index mapping chunk_ids → dense vectors
    ChromaDB Index:
...
[Hybrid Retrieval at Root Node]
    ...
    Stage 2: DPR + ChromaDB ANN Vector Search
        DPR: query encoded with facebook/dpr-question_encoder-single-nq-base
        FAISS inner-product search returns top 50 chunks by DPR score
        ChromaDB: query embedding = all-MiniLM-L6-v2 embedding of query string
        ChromaDB HNSW search over filtered subset returns top 50 chunks by cosine similarity
    Stage 3: RRF Fusion
        Combine DPR ranks and ChromaDB ranks via Reciprocal Rank Fusion
...
[Explanation Engine]
    Per-result Explanation:
        For each retrieved chunk:
            Identify shared features: act_sections, TF-IDF term overlap, party_type match
...
Baselines to beat are: DPR-only retrieval without ChromaDB embedding search. ChromaDB-only retrieval with all-MiniLM-L6-v2 without DPR. Hybrid retrieval with RRF but without MMR. Hybrid retrieval with MMR but without cross-encoder re-ranking. Each baseline is evaluated on the same test set with the same metrics. The expectation is that DPR-only performs well on queries requiring deep semantic understanding but may miss results that benefit from a different embedding perspective. ChromaDB-only performs well on general semantic similarity but lacks the passage-level training signal of DPR. Hybrid without MMR suffers from redundancy in the top K. Hybrid without cross-encoder re-ranking has lower precision because it relies on embedding similarity rather than direct relevance scoring. The full system with all components should achieve the highest scores across all metrics.
...
Phase 1 focuses on foundational data infrastructure. Deliverables: ADX dataset ingestion script with S3 sync or API download, automated retry on network failure, and logging of ingested PDF paths. PDF text extraction pipeline using pdfplumber with multi-column layout handling and header/footer removal. Metadata extraction for case_id, year, bench_type, act_sections, judge_names, outcome_label, validated on a sample of 500 judgments by manual review. DPR index built with facebook/dpr-ctx_encoder-single-nq-base and FAISS, and ChromaDB index built with all-MiniLM-L6-v2 embeddings and HNSW. Baseline hybrid retrieval with RRF fusion tested on a small manually annotated set of 20 query-precedent pairs. Acceptance criteria: retrieval returns results in under 2 seconds per query for a corpus of 10,000 judgments, and manual review confirms correct metadata extraction on 95+ percent of sampled cases.
...
V. Karpukhin, B. Oguz, S. Min, P. Lewis, L. Wu, S. Edunov, D. Chen, and W. Yih, "Dense passage retrieval for open-domain question answering," in *Proceedings of the 2020 Conference on Empirical Methods in Natural Language Processing*, 2020, pp. 6769–6781.
